...
开发工具中的所有 DogSkin 预览都会在应用表情后调用 `DogVisual.ShowInspectionClaws()`，固定显示左侧手背与右侧手心。该检查姿势用于同时观察两种爪子与大尺寸眼镜的交叠，不属于游戏运行时的默认表情。新增预览入口时必须继续调用该方法，不能只在主编辑页特殊处理。

检查姿势必须复用游戏的桌面层级：手心为绝对 Z=1，桌面或替代遮挡层为 Z=2，手背为绝对 Z=3。普通编辑器预览没有真实桌布，因此使用与预览背景同色、从 PSD 桌面线 `y=677` 开始的 `ColorRect` 作为不可见遮挡层。不得把手心提升到 Z=3，否则手心素材的桌下部分会完整露出。

纸箱素材横跨桌面线，不能把整张 PNG 直接作为 Z=2。图标合成会把纸箱拆成两个渲染层：完整纸箱作为 Z=-1 的后层；另一份纸箱清空 `y < 677` 的像素后作为 Z=2 的前层。这样箱体侧翼和内部位于狗狗后方，而 `y >= 677` 的箱体正面仍能遮住手心的桌下部分。
...
flowchart TD
    ...
    G --> H
    H --> I[按狗狗水平中心与顶部安全线取景]
    I --> J[256x256 输出并清空外围 8 像素]
```

纸箱素材保持 1200×1200 完整 PSD 画布，不应预先缩小为 256×256。所有狗狗部件、固定眼镜和纸箱先在源尺寸合成，只在最后执行一次 Lanczos 缩放，避免交界处因多次缩放出现透明虚边。

取景只以狗狗透明像素包围盒为准。狗狗宽度最多 240 像素，高度最多 232 像素，水平中心对齐 256 画布中心，狗狗顶部对齐 16 像素安全线；因此狗狗必须完整落入安全区域，耳朵不会贴近外围 8 像素的强制透明边界。纸箱不参与缩放或纵向取景，可以在画布底部和两侧被裁切。
