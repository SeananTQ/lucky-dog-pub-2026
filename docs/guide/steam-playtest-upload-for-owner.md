...
第二个 Build 应修改一个容易辨认且不破坏存档的内容，再上传到相同测试分支。主人保留或启动第一个 Build 后，在游戏设置页点击“更新游戏”，并检查：

- 当前游戏进程正常退出，Steam 开始校验或下载。
- Steam 完成处理后重新启动游戏，没有被单实例锁拦截。
- 新版本中的辨认标记已经出现。
- 旧存档、装备和关键进度仍然可用。

从 Godot 编辑器点击按钮并成功启动 Steam 库中的游戏，只能证明保存、标记校验、Steam URI 和进程交接链路可用；前后两个 Steam Build 的测试才证明真实差异更新已经完成。
