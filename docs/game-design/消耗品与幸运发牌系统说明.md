...
Refreshment 不作为 Steam Inventory 物品处理。使用、扣除、Buff 持续状态和桌面显示状态先写入账号专属的本地存档；其中 `RefreshmentBlindBox` 来源的消耗品数量额外通过 Steam Cloud API 跨设备同步。Buff 剩余局数、当前桌面 Refreshment 和 New 标记仍只保存在本地。完整边界与冲突策略见 [Steam 云存档与装扮预设设计](Steam云存档与装扮预设设计.md)。
...
`ItemType = Refreshment` 且 `AcquisitionType = RefreshmentBlindBox` 的物品数量同时写入 `consumables_v1.json`。首次建立云端基线时按每种物品的较大数量合并，建立基线后的正常修改按文档更新时间采用较新版本。账号专属本地镜像立即更新，Steam Cloud 写入使用 3 秒防抖并在失败后每 15 秒重试。

云端恢复只替换上述消耗品数量，不恢复 Buff 回合、Buff 参数快照、当前桌面 Refreshment 或 New 标记。
