...
第一版不新增独立 Buff 配置表。所有 Refreshment 使用同一个 Lucky Deal Buff，且使用同一套气球图标和状态图标。

第一版固定规则：

- 所有 `ItemType = Refreshment` 且玩家拥有数量大于 0 的物品都可以摆放。
- 所有 Refreshment 使用后都发放 Lucky Deal Buff。
- Buff 持续局数和触发概率先使用代码中的第一版固定参数。
- 气球效果图固定使用 `Hint_RefreshmentEffect.svg`。
- 确认点击提示图标固定使用 `Icon_CircleCheck.svg`。
- Buff 状态图标固定使用 `Icon_LuckyDealBuff_Color.svg`。

未来当不同 Refreshment 需要不同 Buff 效果、持续局数、触发概率或状态图标时，再新增独立配置表。届时可考虑 `ConsumableBuffConfig` 或 `RefreshmentBuffConfig`。
