...
设置页不再提供普通的本地进程重启，原位置已替换为“更新游戏”。该入口先调用 `SteamApps.MarkContentCorrupt(false)`，再通过 Steam URI 重新启动，因此单实例锁的交接对象是 Steam 随后创建的新游戏进程。

当前流程为：

1. 立即保存本地状态。
2. 请求 Steam 在下次启动时完整校验已安装内容。
3. 释放单实例 Mutex。
4. 打开 `steam://run/<当前运行 AppID>`。
5. Steam URI 打开成功后退出旧进程。
6. 打开失败时尝试重新取得 Mutex，并保持旧游戏继续运行。

AppID 取自当前真实 Steam 会话，不写死 Playtest 或正式版编号。当前实现不比较 BuildID，不启用后台自动检查，也不创建独立 Helper 进程。Steam Mock、显式离线模式或没有有效 AppID 时不会执行该流程。
...
- 主人已在 Godot 编辑器的真实 Steam 环境中点击“更新游戏”，确认编辑器 Runtime 退出后 Steam 库中的游戏成功启动。

尚未完整人工确认：

- 主场景处于不同显示模式或全屏隐藏状态时，重复启动后的窗口唤醒表现。
- 同一测试分支上传前后两个 Build 后，“更新游戏”触发 Steam 完整校验、差异下载并启动新版的完整链路。
- Playtest 和 Release 导出包中的重复启动表现。

这些项目不是当前功能收束的阻塞条件。主人后续遇到重复启动、更新重启或打包测试场景时继续观察即可；出现异常时根据本文档中的日志和进程检查方式继续定位。
