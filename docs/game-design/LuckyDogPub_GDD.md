...
> 状态：企划草案
>
> 本文保留早期玩法与美术构想，不作为当前装扮系统的实现依据。文中“眼镜作为独立装扮、拒绝表现或里程碑奖励”的设计已经废弃；当前眼镜素材属于 `DogSkin` 的固定组成部分，独立眼镜物品只作为已下架历史数据保留。现行规则以[物品默认穿戴规则](./物品默认穿戴规则.md)和[盲盒系统设计](./盲盒系统设计.md)为准。
...
当前实现以任意点数的一对作为最低中奖牌型，不使用 Jacks or Better 的 J、Q、K、A 高对门槛。牌型判定以 `CardEvaluator` 为准，赔率以 Luban 导出的 `tbpaytable.json` 为准。
...
下注规则：**固定每局 50 筹码**，新建或重置本地存档时给玩家 **2800 筹码**。筹码不足时牌局等待玩家通过全局输入补足余额，不触发旧版 Game Over 重置流程。
