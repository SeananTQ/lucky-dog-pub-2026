...
当前 LinkTree 页面中的“Steam 奖励暂时不可用”只在玩家主动打开 LinkTree 时出现，不是启动等待超时提示；盲盒上的 `STEAM` 或 `FALLBACK` 标签也不是该提示。

Dev 调试页中的“显示 Steam Mock 面板”只控制普通扑克模式上方的会话级面板，不保存设置。Mock 通过真实平台接口事件驱动正式盲盒事务，但不会向 Steam 写库存或成就；激活期间 LinkTree 真实领奖被禁止。`Scenes/Debug/*` 与所有 `DEBUG` 条件类型不会进入 Playtest/Release。
