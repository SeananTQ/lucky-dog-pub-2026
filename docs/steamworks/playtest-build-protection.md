...
- Playtest 有效期截止到香港时间 2026-08-11 结束；从 2026-08-12 00:00 起弹出过期提示并退出。Dev 和 Release 永不过期。
...
EXE 名固定为 `LuckyDogRise.exe`，包名为 `LuckyDogRise-<version>-<channel>-win-x64.zip`。Godot .NET 运行目录 `data_LuckyDogRise_windows_x86_64` 必须与 EXE 一起分发并保持目录结构。

Windows 版本资源由脚本统一写入：

- Company：`Seanan Studio`
- Product：`Lucky Dog Rise`
- File description：`Lucky Dog Rise`
- File version：项目版本补足为四段，例如 `0.3.1.0`
- Product version：项目版本，例如 `0.3.1`
- Icon：`res://windows_icon.ico`
...
成功后只发送 `GameBuild/LuckyDogRise-<version>-playtest-win-x64.zip`，不要发送 `.local-build`、源码目录或 staging 目录。
...
- `LuckyDogRise.exe` 已通过加密包启动测试，Windows 文件属性已验证为 `Seanan Studio / Lucky Dog Rise / 0.3.1.0 / 0.3.1`。
- Playtest 到期时间已作为 assembly metadata 注入；当前包在有效期内通过启动验证。

当前产物：`GameBuild/LuckyDogRise-0.3.1-playtest-win-x64.zip`。
