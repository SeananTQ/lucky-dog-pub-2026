...
```
改优先级只改对应数组顺序，不改 `GetPanelSlotPosition()`。兜底策略：所有完整槽位都不可用时，若 5 区覆盖 A 区后仍能完整显示标题栏、关闭按钮和顶部操作按钮，则继续使用 5 区；否则按 `2 → 1 → 3` 尝试允许底部超屏的槽位，并保证上述关键操作区可见；仍无法满足时以 2 区为基础夹紧关键操作区坐标。

桌宠模式的 4 宫和 6 宫以 `ContentA/TaskBar` 锚点作为面板底边，不使用整个缩放后 A 区的底边；这样高倍率下系统功能面板不会被额外向下推。扑克模式仍使用扑克 A 区底边。
